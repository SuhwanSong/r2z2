...
class Bisector(threading.Thread):
    def __init__(self, id_, start_pos, end_pos, mode, br_type):
    ...
    def run(self):
        ...
        while True:
            ...
            if not Queues: 
                ...
                print (br_locs)
                if self.__bisector is not None:
...
#    set_affinity(range(int(count_cpu() / 2), count_cpu()))
    ...
    parser.add_argument('-s', '--start', required=True, type=int, help='')
    parser.add_argument('-e', '--end', required=True, type=int, help='')
    ...
    parser.add_argument('--firefox', action="store_true", default=False, help='')
...
#        num_proc = 1
        ...
        for path in tmp:
            dirname = get_parent_dirname(path)
            if not dirname.isnumeric():
                continue
            if os.path.islink(path):
                print ('{} is passed'.format(dirname))
                continue
            vv = int(dirname)
            if start_commit <= vv and vv <= end_commit:
                br_lists.append(path)
    ...
    for idx in range(num_proc):
        bisectthread = Bisector(idx, bb, ee, mode, br_type)
...
#    summarize(output_dir)
    ...
    write_file(os.path.join(output_dir, 'bisected-list.txt'), write_tmp_buffer)
